    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await this.userService.CreateAsync(user, model.Password, model.Roles == null ? new[] { ApplicationRoles.User } : model.Roles.Where(r => r.IsSelected).Select(r => r.Name).ToArray());
                if (result.Succeeded)
                {
